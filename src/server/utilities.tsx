...
export async function analyzeImageWithGPT(imageBase64: string): Promise<string> {
  try {
...
export async function generateMorningBriefMarkdown(analyses: string[]): Promise<string> {
  try {
    ...
    const response = await openai.chat.completions.create({
      ...
      messages: [
        {
          role: "system",
          content: "You are a professional brief writer. Create concise, well-structured morning briefs in markdown format."
        },
        {
          role: "user",
